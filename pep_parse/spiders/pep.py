...
class PepSpider(scrapy.Spider):
    ...
    start_urls = [f"https://{i}/" for i in allowed_domains]
    ...
    def parse_pep(self, response):
        ...
        number = int(
            response.xpath("/html/body/section/header/ul/li[3]/text()")
            .get()
            .split(" ")[1]
        )
        ...
        yield PepParseItem(status=status, number=number, name=name)
